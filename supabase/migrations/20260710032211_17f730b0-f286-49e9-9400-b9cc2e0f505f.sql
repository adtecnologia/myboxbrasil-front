...
BEGIN
  ...
  IF v_cidade IS NULL OR v_estado IS NULL THEN
    RETURN jsonb_build_object(
      ...
      'top_bairros', '[]'::jsonb, 'pedidos_recentes', '[]'::jsonb,
      'ordens_por_mes', '[]'::jsonb, 'classes_residuo', '[]'::jsonb
    );
  END IF;
  ...
  WITH obras_cidade AS (
    ...
    WHERE lower(cidade) = lower(v_cidade) AND upper(estado) = upper(v_estado)
  ),
  ord AS (
  ...
  stats AS (
    SELECT
      ...
      (SELECT COUNT(*)
         ...
         WHERE c.locador_id IN (SELECT user_id FROM locadores_cid)
      ) AS cacambas,
  ...
  classes AS (
    ...
    GROUP BY 1 ORDER BY volume DESC LIMIT 8
  ),
  pedidos_status AS (
    SELECT pedido_id,
           CASE
  ...
  SELECT jsonb_build_object(
    ...
    'pedidos_recentes', COALESCE((
      SELECT jsonb_agg(row_to_json(x)) FROM (
        SELECT DISTINCT p.id, p.numero, ps.status, p.valor_total, p.created_at, prof.nome AS locatario_nome
    ...
    'ordens_por_mes', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('mes', mes, 'value', total) ORDER BY mes) FROM (
        SELECT to_char(date_trunc('month', created_at), 'YYYY-MM') AS mes, COUNT(*)::int AS total
        FROM ord_ano GROUP BY 1
      ) m
    ), '[]'::jsonb)
