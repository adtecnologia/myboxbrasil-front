...
CREATE OR REPLACE FUNCTION public.get_prefeitura_dashboard(
...
BEGIN
  ...
  -- Faixa de datas para filtros de stats/pedidos/top_bairros.
  -- Se _mes e _ano vierem, filtra pelo mês; se só _ano vier, filtra o ano todo.
  IF _ano IS NOT NULL AND _mes IS NOT NULL THEN
    v_start := make_timestamptz(_ano, _mes, 1, 0, 0, 0);
    v_end   := v_start + INTERVAL '1 month';
  ELSIF _ano IS NOT NULL THEN
    v_start := make_timestamptz(_ano, 1, 1, 0, 0, 0);
  ...
  ord AS (
    ...
    WHERE ol.obra_id IN (SELECT id FROM obras_cidade)
      AND (v_start IS NULL OR ol.created_at >= v_start)
      AND (v_end   IS NULL OR ol.created_at <  v_end)
  ),
  ord_ano AS (
    -- Sempre agrupa por mês do ano selecionado (ou ano corrente) para o gráfico
