...
const SituacaoLocadores = () => {
  ...
  const { data: rows = [], isLoading } = useQuery<SituacaoLocador[]>({
    queryKey: ["situacao-locadores", userId],
    enabled: !!userId,
    queryFn: async () => {
      ...
      return list.map((l) => {
        ...
        let status: SituacaoLocador["status"];
        if (l.licenca_status === "validado" && vencidos === 0) status = "Regular";
        else if (l.licenca_status === "rejeitado" || vencidos > 0) status = "Irregular";
        else status = "Aguardando Validação";
        return {
          id: l.id,
          nome: l.nome ?? "—",
  ...
  const handleExportExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Situação Locadores");
    XLSX.writeFile(wb, `situacao-locadores-${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
  ...
  const handlePrint = () => {
    ...
    const rowsHtml = filtered
      ...
      .join("");
    w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Situação Locadores</title>
      <style>
      ...
      <h1>Situação Locadores</h1>
      <div class="meta">Validação de documentos e regularidade</div>
      <table>
        <thead><tr><th>Nome</th><th>CPF/CNPJ</th><th>Docs Vencidos</th><th>Situação</th></tr></thead>
