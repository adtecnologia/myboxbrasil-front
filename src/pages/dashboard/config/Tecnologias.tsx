  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between rounded-xl bg-gradient-to-r from-primary to-[hsl(155,45%,40%)] p-5 sm:p-6 text-primary-foreground">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold">Tecnologias de Tratamento</h1>
          <p className="text-sm text-white/75">Gestão de métodos de processamento de resíduos</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={(open) => {
          ...
          <DialogTrigger asChild>
            <Button className="bg-white/20 hover:bg-white/30 text-white border-0">
              <Plus className="mr-2 h-4 w-4" /> Nova Tecnologia
            </Button>
          </DialogTrigger>
          <DialogContent>
            <DialogHeader>
      ...
      <DataTable<Tecnologia>
        title={`${tecnologias.length} tecnologias cadastradas`}
        data={paginatedData}
        searchValue={search}
        onSearchChange={setSearch}
        columns={[
          { header: "Tratamento", accessor: "nome", className: "font-medium w-1/3" },
...
export default TecnologiasTratamento;