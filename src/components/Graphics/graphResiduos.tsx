...
// SERVIÇOS
import { cor2, cor3, cor4, GET_API } from "../../services";

// INTERFACE
interface GraphResiduosPorMesInterface {
    filters?: any,
    height?: string
}
...
const GraphResiduosPorMes = ( { filters, height } : GraphResiduosPorMesInterface ) => {
    ...
    useEffect(() => { 
        setLoading(true);
        GET_API(`/dashboard/residuebymonth?ref=${filters.filterAno.value}-${filters.filterMes.value}`).then(rs => rs.json()).then(res => setData(res.data)).finally(() => setLoading(false))
    }, [filters])
...
export default GraphResiduosPorMes