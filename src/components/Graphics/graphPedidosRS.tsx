...
const GraphPedidosRS = ( { filters, height } : GraphPedidosRSInterface ) => {
    ...
    useEffect(() => {
        ...
        if (chart) {
            ...
            const option: any = {
                ...
                yAxis: {type: 'value', axisLabel: { formatter: (value:any) => `R$ ${Number(value).toLocaleString('pt-br')}` } },
                tooltip: { trigger: 'axis', position: function (pt:any) { return ['10%', '10%']; }, valueFormatter: (value:any) => `R$ ${Number(value).toLocaleString('pt-br')}` },
                grid: { left: '0px', right: '20px', bottom: '10px', top: '20px', containLabel: true },
                dataZoom: [ { type: 'inside', start: 0, end: '100%' } ],
                lineStyle: { color: cor3, width: 2 },
                series: [
                    { name: 'Faturamento', type: 'line', smooth: true, symbol: 'none', areaStyle: { 
                        opacity: 0.8,
    ...
    useEffect(() => { 
        setLoading(true);
        GET_API(`/financial/orderbymonth?ref=${filters.filterAno.value}-${filters.filterMes.value}`).then(rs => rs.json()).then(res => setData(res.data)).finally(() => setLoading(false))
    }, [filters])
