...
const GraphPedidos = ( { filters, height } : GraphPedidosInterface ) => {
    ...
    useEffect(() => {
        ...
        if (chart) {
            ...
            const option: any = {
                ...
                series: [
                    { name: 'Total de ordens de locação', type: 'line', smooth: true, symbol: 'none', areaStyle: { 
                        opacity: 0.8,
    ...
    useEffect(() => { 
        setLoading(true);
        GET_API(`/dashboard/orderbymonth?ref=${filters.filterAno.value}-${filters.filterMes.value}`).then(rs => rs.json()).then(res => setData(res.data)).finally(() => setLoading(false))
    }, [filters])
