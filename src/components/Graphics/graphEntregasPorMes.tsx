...
// INTERFACE
interface GraphEntregasPorMesInterface {
    filters?: any,
    height?: string
}
...
const GraphEntregasPorMes = ( { filters, height } : GraphEntregasPorMesInterface ) => {
    ...
    useEffect(() => { 
        setLoading(true);
        GET_API(`/dashboard/deliverybymonth?ref=${filters.filterAno.value}-${filters.filterMes.value}`).then(rs => rs.json()).then(res => setData(res.data)).finally(() => setLoading(false))
    }, [filters])
...
export default GraphEntregasPorMes