...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      <DialogPrimitive.Close className="absolute right-6 top-6 rounded-sm text-primary-foreground/80 ring-offset-background transition-opacity hover:text-primary-foreground focus:outline-none focus:ring-2 focus:ring-primary-foreground/40 focus:ring-offset-2 disabled:pointer-events-none">
        <X className="h-4 w-4" />
...
const DialogHeader = ({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) => (
  <div
    className={cn(
      "p-6 flex flex-col space-y-1 text-left",
      "bg-gradient-to-r from-primary to-[hsl(155,45%,40%)] text-primary-foreground",
...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Description ref={ref} className={cn("text-xs text-primary-foreground/80", className)} {...props} />
));
