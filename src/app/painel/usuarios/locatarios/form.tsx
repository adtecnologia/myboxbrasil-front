...
const TenantdForm = ({ type, path, permission }: PageDefaultProps) => {
    ...
    const onSend = (values: any) => {
        ...
        fromAddress(address).then(({ results }) => {
            ...
            POST_API(`/user`, {...values, profile_type: doc === 'cpf' ? 'CUSTOMER' : 'LEGAL_CUSTOMER'}, ID).then(async (rs) => {
                if (rs.ok) return rs.json();
                else {
                    if (rs.status === 422) {
                        const res = await rs.json()
                        Modal.warning({ title: "Algo deu errado", content: res.message })
