...
const Order = ({ type, path, permission }: PageDefaultProps) => {
  ...
  const column = [
      ...
      render: (item: any) => (
        <Row justify={'center'} style={{ width: '100%' }}>
          ...
          <Tag color={item.status.color} style={{ textAlign: 'center' }}>
            {item.status.name} <br />
            {item.status.code !== 'AR' ? item.updated_at : null}
          </Tag>
        </Row>
      ),
    },
    ...
    {
      title: 'Quantidade',
      dataIndex: 'cart_product.quantity',
      ...
      render: (item: any) => (
        <Row style={{ width: '100%', flexDirection: 'column' }}>
          <Col>
            ...
            >
              <span>
                Modelo {item.cart_product.product.stationary_bucket_type.name}
              </span>
            </Typography>
          </Col>
          {item?.items.map((v: any, i: any) => (
            <Col>
