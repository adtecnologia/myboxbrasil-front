...
const TecnologieList = ({ type, path, permission } : PageDefaultProps ) => {
    ...
    // table columns
    const column = [
        { title: 'Tratamento', dataIndex: 'name', table: 'name', width: '300px', sorter: true, align: 'left', render: null },
        { title: 'Descrição', dataIndex: 'description', table: 'description', width: 'auto', minWidth: '100px', sorter: true, align: 'left', render: null },
    ...
    return (
        <PageDefault valid={`${permission}.${type}`} items={[
            { title: <Link to={ type === 'list' ? '#' : '..' }>Tecnologias de Tratamento</Link> },
