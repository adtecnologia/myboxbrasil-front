...
const DashFornecedor = ({ filters }: DashFornecedorInterface) => {
  ...
  useEffect(() => {
    setDisponiveis(-1);
    GET_API(
      `/stationary_bucket?status=D&ref=${filters.filterAno.value}-${filters.filterMes.value}`
    )
      .then((rs) => rs.json())
  ...
  useEffect(() => {
    setEntrega(-1);
    GET_API(
      `/stationary_bucket?status=EP&ref=${filters.filterAno.value}-${filters.filterMes.value}`
    )
      .then((rs) => rs.json())
  ...
  useEffect(() => {
    setLocadas(-1);
    GET_API(
      `/stationary_bucket?status=L&ref=${filters.filterAno.value}-${filters.filterMes.value}`
    )
      .then((rs) => rs.json())
  ...
  useEffect(() => {
    setRetirada(-1);
    GET_API(
      `/stationary_bucket?status=AR&ref=${filters.filterAno.value}-${filters.filterMes.value}`
    )
      .then((rs) => rs.json())
  ...
  useEffect(() => {
    setTransito(-1);
    GET_API(
      `/stationary_bucket?statusIn=ETR,ETL&ref=${filters.filterAno.value}-${filters.filterMes.value}`
    )
      .then((rs) => rs.json())
  ...
  useEffect(() => {
    setLimpeza(-1);
    GET_API(
      `/stationary_bucket?status=ML&ref=${filters.filterAno.value}-${filters.filterMes.value}`
    )
      .then((rs) => rs.json())
