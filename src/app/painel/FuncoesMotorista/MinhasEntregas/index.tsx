...
const MinhasEntregas = ({ type, path, permission }: PageDefaultProps) => {
  ...
  const column = [
    {
      ...
      table: 'order_location_products.delivery_location_date',
      width: '180px',
      sorter: true,
      align: 'center',
      render: (item: any) => (
        <Row style={{ width: '100%' }}>
          <Col span={24}>
            <Typography style={{ textAlign: 'center' }}>
              {item.delivery_location_date_format}
            </Typography>
            <Typography
  ...
  const reader = (result: any) => {
    if (typeLoad === 'EP') {
      GET_API(`/order_location_product?code=${result[0].rawValue}&status=EP`)
        .then((rs) => rs.json())
        .then((res) => {
  ...
  const readerWrite = (result: any) => {
    if (typeLoad === 'EP') {
      GET_API(`/order_location_product?code=${result}&status=EP`)
        .then((rs) => rs.json())
        .then((res) => {
          if (res.data.length > 0) {
            onProduct(res.data[0]);
  ...
  const onSend = async (values: any) => {
    ...
    try {
      await product.forEach((item) => {
        POST_API('/order_location_product', { status: 'ETL' }, item.id)
          .then((rs) => {
            if (rs.ok) {
              loadDelivery();
            } else {
  ...
  const onFinish = () => {
    setLoadingButton(true);
    POST_API('/order_location_product', { status: 'L' }, productSelect.id)
      .then((rs) => {
        if (rs.ok) {
  ...
  const loadDelivery = () => {
    GET_API('/order_location_product?status=ETL&driver=true')
      .then((rs) => rs.json())
      .then((res) => {
        if (res.data.length > 0) {
          setTypeLoad('ETL');
    ...
    <PageDefault items={[{ title: 'Entregas Agendadas' }]} valid={'fmt.vep'}>
      {typeLoad ? (
        <Row gutter={[16, 16]}>
          ...
          <Col md={8} xs={24}>
            <CardItem
              title={`${typeLoad === 'EP' ? 'Iniciar entrega' : 'Entrega iniciada'} | ${product.length} selecionado(s)`}
            >
              <Radio.Group
          ...
          <Col md={24} xs={24}>
            <CardItem>
              <Table
                action={action}
                column={column}
                defaultFilter={{ statusIn: ['EP', 'ETL'], driver: true }}
