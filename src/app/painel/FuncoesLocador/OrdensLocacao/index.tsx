...
import { Col, Image, Modal, Row, Segmented, Tabs, TabsProps, Tag, Typography } from "antd";
...
const OrdemLocacaoList = ({  } ) => {
    ...
    const openGallery = (item:any) => {
        setGallery([])
        onModalGallery()
        GET_API(`/order_location_product_gallery?orderLocationProductId=${item.id}&status=L`).then(rs => rs.json()).then((res:any) => {
            setGallery(res.data)
        }).catch(POST_CATCH)
    }
    ...
    return (
        <PageDefault valid={`vlc.list`} items={[
            { title: 'Locadas' }
        ]}>
            <Row gutter={[16,16]}>
                <Col xs={24}>
                    <CardItem>
                        <Tabs items={items} className="ordem-tabs"/>
                    </CardItem>
                </Col>
                <Col xs={24} md={24}>
                    <CardItem>
...
export default OrdemLocacaoList;