...
const EntregasPendentesList = ({ type, path, permission } : PageDefaultProps ) => {
    ...
    const column = [
        ...
        { title: 'Cliente', dataIndex: 'CLIENT_NAME', table: 'order_locations.id', width: 'auto', minWidth: '300px', sorter: true, align: 'left', render: (item:any) => (
            <Row style={{width: '100%'}}>
                <Col span={24}>
        ...
        { title: 'Ações', dataIndex: null, width: '100px', sorter: false, align: 'center', render: (item: any) => (
            <Row justify={'center'} style={{width: '100%'}}>
                { verifyConfig(['vep.edit']) ? product.filter((v) => Number(v.id) === Number(item.id)).length > 0 ? <Col><MdOutlineCheckBox onClick={() => onProduct(item)} size={18} className="actions-button"/></Col>
    ...
    return (
        <PageDefault valid={`vep.list`} items={[
            { title: 'Entregas Pendentes' }
        ]}>
            <Row gutter={[16,16]}>
                <Col xs={24} md={ verifyConfig(['vep.edit']) ? 16 : 24 }>
                    <CardItem>
                        { coord ? loadMap ?
                ...
                <Col xs={24} md={ verifyConfig(['vep.edit']) ? 8 : 24 }>
            ...
            <MapFullScreen drive={true} startStatus={['EP']} open={open} setOpen={setOpen} />
        </PageDefault>
    )
